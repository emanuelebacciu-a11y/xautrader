...
try { *hapticCtx = new (window.AudioContext || window.webkitAudioContext)(); } catch(*){}
}
return _hapticCtx;
};
...
try { localStorage.setItem(‘xt_sb_cache’, JSON.stringify({ trades: grouped, equity, ts: Date.now() })); } catch(*) {}
...
const removeWithTF = (idx) => update(d => ({ …d, withTF: d.withTF.filter((*,i)=>i!==idx) }));
...
…d, withTF: d.withTF.map((x,i)=>i===idx ? {…x,tfs:x.tfs.filter((*,j)=>j!==tfi)}:x)
}));
...
const removeVWAP = (idx) => update(d => ({ …d, vwap: d.vwap.filter((*,i)=>i!==idx) }));
...
…d, vwap: d.vwap.map((x,i)=>i===idx ? {…x,tfs:x.tfs.filter((*,j)=>j!==tfi)}:x)
}));
...
const proj = Array.from({length:30},(*,i)=>({day:`+${i+1}g`,value:null,projection:Math.round(last+g*(i+1))}));
...
return [
…Array.from({length:Math.min(maxW,7)},(*,i)=>({len:`${i+1}W`,count:wins[i+1]||0,type:‘win’})),
…Array.from({length:Math.min(maxL,5)},(*,i)=>({len:`${i+1}L`,count:losses[i+1]||0,type:‘loss’})),
].filter(d=>d.count>0);
})();
...
// Prova direttamente Yahoo v8 con cookie/crumb (funziona spesso dal browser)
const tryDirect = async () => {
// Step 1: ottieni crumb
const crumbRes = await fetch(‘https://query1.finance.yahoo.com/v1/test/getcrumb’, {
credentials: ‘include’,
headers: { ‘User-Agent’: ‘Mozilla/5.0’ },
});
const crumb = crumbRes.ok ? await crumbRes.text() : null;
const crumbParam = crumb ? `&crumb=${encodeURIComponent(crumb.trim())}` : ‘’;
const url = `https://query1.finance.yahoo.com/v8/finance/chart/${symbol}?interval=${interval}&range=${range}&includePrePost=false${crumbParam}`;
const res = await fetch(url, { credentials: ‘include’, headers: { ‘User-Agent’: ‘Mozilla/5.0’ } });
if (!res.ok) throw new Error(`YF direct ${res.status}`);
return res.json();
};
...
async () => {
const res = await fetch(`https://thingproxy.freeboard.io/fetch/${yfUrl}`);
...
lastErr = e;
console.warn(’[Chart] proxy failed:’, e.message);
}
}
